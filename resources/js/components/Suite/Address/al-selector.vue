<template>
  <div class="address-main">
    <el-row :gutter="gutterNum" style="margin: 0">
      <el-col v-if="show(0)" :span="span" style="padding: 0">
        <el-select
          ref="prov"
          v-model="currPro"
          popper-class="address address-prov"
          clearable
          :filterable="searchable"
          :placeholder="phHandled(0)"
          :size="size"
          :not-data-text="ndsHandled(0)"
          :disabled="disabled === undefined ? false : disHandled(0)"
          transfer
          style="width:100%"
          @change="hasChange"
        >
          <el-option v-for="item in provList" :key="item" :value="item">{{ item }}</el-option>
        </el-select>
      </el-col>
      <el-col v-if="show(1)" :span="span">
        <el-select
          ref="city"
          v-model="currCit"
          popper-class="address address-city"
          clearable
          :filterable="searchable"
          :placeholder="phHandled(1)"
          :size="size"
          :not-data-text="ndsHandled(1)"
      ...
      <el-col v-if="show(2)" :span="span">
        <el-select
          ref="coun"
          v-model="currCou"
          popper-class="address address-coun"
          clearable
          :filterable="searchable"
          :placeholder="phHandled(2)"
          :size="size"
          :not-data-text="ndsHandled(2)"
          :disabled="disabled === undefined ? false : disHandled(2)"
          transfer
          style="width:100%"
          @change="hasChange"
        >
          <el-option v-for="item in counList" :key="item" :value="item">{{ item }}</el-option>
        </el-select>
      </el-col>
      <el-col v-if="show(3)" :span="span">
        <el-select
          ref="stre"
          v-model="currStr"
          popper-class="address address-stre"
          clearable
          :filterable="searchable"
          :placeholder="phHandled(3)"
          :size="size"
          :not-data-text="ndsHandled(3)"
          :disabled="disabled === undefined ? false : disHandled(3)"
          transfer
          style="width:100%"
          @change="hasChange"
        >
          <el-option v-for="item in streList" :key="item" :value="item">{{ item }}</el-option>
        </el-select>
      </el-col>
    </el-row>
    <slot />
    ...
    updateNextSelector(index, list, nextList, name, nextName, level) {
      if (level <= this.showLevel) {
        ...
        if (areaData[this[index]]) {
          ...
          // if (this.isInit && this.value.length !== 0 && this.value[level]) {
